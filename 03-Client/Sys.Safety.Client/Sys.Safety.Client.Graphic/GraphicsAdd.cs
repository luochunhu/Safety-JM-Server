...
{
    public partial class GraphicsAdd : DevExpress.XtraEditors.XtraForm
    {
        /// <summary>
        /// 系统默认图形id
        /// </summary>
        ...
        {
            Program.main.GraphOpt.IsTopologyInit = false;
            ...
            try
            {
                ...
                if (comboBoxType.SelectedItem.ToString() == "动态图" || comboBoxType.SelectedItem.ToString() == "SVG组态图")
                {
                ...
                if (this.Text == "新建图形")
                {
                    ...
                    {
                        Program.main.GraphOpt.UpdateEmergencyLinkageGraphicsRequest(this.EmergencyLinkageGraphicsId);
                    }
                    if (Type == 0 )
                    {
                        //查找目录下是否具有同名文件
                        ...
                        if (this.checkEdit2.Checked == true && this.checkEdit2.Text == "通风系统默认图形")
                        {
                            ...
                            {
                                Program.main.GraphOpt.UpdateSystemDefaultGraphics("0", this.SysetemDefaultGraphicsId);
                            }
                        ...
                        Program.main.GraphOpt.GraphInsert(GraphFileName, Type, bz3, bz4, File.ReadAllBytes(textFileName.Text));
                    }
                    else
                    {
                        //查找数据库中是否有相同图形
                        GraphicsbaseinfInfo GraphicsbaseinfDTO_ = Program.main.GraphOpt.getGraphicDto(GraphFileName);
                        ...
                        else
                        {
                            if (comboBoxType.SelectedItem.ToString() == "SVG组态图")
                            {
                                ...
                                //    bz4 = "0";
                                //}
                                Program.main.GraphOpt.GraphInsert(GraphFileName, Type, bz4, File.ReadAllBytes(textFileName.Text), "SVGLoad.html", GraphFileName);
                            }
                            else
                            {
                                string bz3 = "0", bz4 = "0";
                                ...
                                //    bz4 = "0";
                                //}
                                Program.main.GraphOpt.IsTopologyInit = true;
                                Program.main.GraphOpt.GraphInsert(GraphFileName, Type, bz3, bz4, null);
                            }
                        }
                    }
                }
                else if (this.Text == "图形编辑")
                {
                    ...
                    if (comboBoxType.SelectedItem.ToString() == "SVG组态图")
                    {
                        Program.main.GraphOpt.GraphUpdate(textGraphName.Text, GraphFileName, Type, File.ReadAllBytes(textFileName.Text), "SVGLoad.html", textGraphName.Text);
                    }
                    else
                    {
                        string bz3 = "0", bz4 = "0";
                        ...
                        {
                            //更新动态图形
                            Program.main.GraphOpt.GraphUpdate(textGraphName.Text, GraphFileName, Type, bz3, bz4, File.ReadAllBytes(textFileName.Text));
                        }
                        else if (Type == 1)
                        {
                            //修改拓扑图形
                            Program.main.GraphOpt.GraphUpdate(textGraphName.Text, GraphFileName, Type, bz3, bz4, null);
                        }
                    }
                }
                if (Type == 0 || Type == 3)//只有动态图、SVG图才进行底图保存
                {
                    if (SGraphFileName != textFileName.Text)
                    {
                        //保存本地
        ...
        private void GraphicsAdd_Load(object sender, EventArgs e)
        {
            try
            {
                if (this.Text == "图形编辑")
                {
                    ...
                    if (Program.main.GraphOpt.getGraphicNowType() == 0)
                    {
                        comboBoxType.SelectedItem = "动态图";
                    }
                    else if (Program.main.GraphOpt.getGraphicNowType() == 1)
                    {
                        comboBoxType.SelectedItem = "拓扑图";
                    }
                    else if (Program.main.GraphOpt.getGraphicNowType() == 3)
                    {
                        comboBoxType.SelectedItem = "SVG组态图";
                    }

                    var graphicOperations = new GraphicOperations();
                    var graphicInfo = graphicOperations.getGraphicDto(Program.main.GraphOpt.GraphNameNow);
                    if (graphicInfo != null)
                    {
                    ...
                    string graphFileName = "", graphNameNow = Program.main.GraphOpt.GraphNameNow;
        ...
        private void checkEdit3_CheckedChanged(object sender, EventArgs e)
        {
            if (this.Text == "新建图形")
            {
                ...
                var graphicsInfo = Program.main.GraphOpt.GetEmergencyLinkageGraphics();
        ...
        private void checkEdit2_Click(object sender, EventArgs e)
        {
            if (this.Text == "新建图形")
            {
                ...
                if (comboBoxType.SelectedItem.ToString() == "动态图")
                {
                    ...
                    graphicsInfo = Program.main.GraphOpt.GetSystemtDefaultGraphics(0);
                }
                else if (comboBoxType.SelectedItem.ToString() == "拓扑图")
                {
                    message = "已存在拓扑定义默认图形，确定要把当前图形设置为拓扑定义默认图形吗?";
                    graphicsInfo = Program.main.GraphOpt.GetSystemtDefaultGraphics(1);
                }
                if (graphicsInfo != null && !string.IsNullOrWhiteSpace(graphicsInfo.GraphId))
                {
