...
{
    public partial class Mnl_McBigDataLine : XtraForm
    {
        ...
        public Mnl_McBigDataLine(Dictionary<string, string> param)
        {
            SetStyle(ControlStyles.UserPaint, true);
            SetStyle(ControlStyles.AllPaintingInWmPaint, true); // 禁止擦除背景.
            SetStyle(ControlStyles.DoubleBuffer, true); // 双缓冲
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DataTable tempdt = dt_line.Clone();
                ...
                for (DateTime stime = SzNameS; stime <= SzNameE; stime = stime.AddSeconds(2))
                {
                    ...
                    if (dr.Length > 0)
                    {
                        ...
                        {
                            object[] obj = new object[dr[0].ItemArray.Length];
                        ...
                        else
                        {
                            ...
                            if (dr1.Length < 1)
                            {
                                tempdt.Rows.Add(dr[0].ItemArray);
                            }
                        }
                    }
                }
                ...
                var _minX = new DateTime(dateEdit1.DateTime.Year, dateEdit1.DateTime.Month, dateEdit1.DateTime.Day, 0, 0, 0);
                var _maxX = new DateTime(dateEdit2.DateTime.Year, dateEdit2.DateTime.Month, dateEdit2.DateTime.Day, 23, 59, 59);
        ...
        private DataTable DataFilter(DataTable oldDt, Jc_DevInfo dev)
        {
            ...
            if (dev.Bz3 == 2 || dev.Bz3 == 9 || dev.Bz3 == 4)//风速、氧气、温度按4%变化进行过滤  20171222
            {
                ChangeRate = 0.04;
            }
            ...
            for (int i = 0; i < oldDt.Rows.Count; i++)
            {
                ...
                if (double.TryParse(dr["A"].ToString(), out tempValue) && dr["state"].ToString() == "21")//正常数据才处理，其他数据直接显示
                {
                        ...
                        if (drs.Length > 0)
                            ...
                            else
                            {
                                if (point.Values[0].ToString() == "1E-05")
                                    ...
                                    ShowText = element.Series.Name + ":" + "未知";
                                else
                                {
        ...
        private void gridView2_RowCellStyle(object sender, RowCellStyleEventArgs e)
        {
            try
            {
                ...
                var typetext = view.GetRowCellDisplayText(e.RowHandle, view.Columns["typetext"]);
                var statetext = view.GetRowCellDisplayText(e.RowHandle, view.Columns["statetext"]);
                if (statetext.Contains("标校"))
                {
                    e.Appearance.BackColor = Color.Blue;
                }
                else if (typetext.Contains("突变"))
                {
                    e.Appearance.BackColor = Color.Orange;
                }
                if (typetext.Contains("上限报警") || typetext.Contains("上限断电") || typetext.Contains("下限断电") || typetext.Contains("下限报警"))
                {
                    e.Appearance.ForeColor = Color.Red;
                }
            }
