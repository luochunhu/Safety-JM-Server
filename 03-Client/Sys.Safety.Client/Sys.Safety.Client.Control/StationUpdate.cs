...
{
    public partial class StationUpdate : Form
    {
        IStationUpdateService stationUpdateService = null;

        uint _crc = 0;
        int _bufferCount = 0;
        byte[] _SendBuffer = new byte[1];
        public StationUpdate()
        {
            InitializeComponent();
        }

        private void StationUpdate_Load(object sender, EventArgs e)
        {
            stationUpdateService = ServiceFactory.Create<IStationUpdateService>();
            LoadStation();
            cb_refrush.Checked = true;
        }
        ...
        private void btn_view_Click(object sender, EventArgs e)
        {
            ...
            if (ofd.FileName != "")
            {
                txt_sb.Text = ofd.FileName;
                _SendBuffer = TxtRead(ofd.FileName, ref _crc,ref _bufferCount);
               
            }
        }

        private byte[] TxtRead(string filename,ref uint crc,ref int bufferCount)
        {
            ...
            try
            {
                ...
                if (checksize > 0)
                {
                    bufferCount = buf.Length / 256;//下发总帧数
                    if ((buf.Length % 256) != 0)
                    {
                        bufferCount++;//下发总帧数
                    }
                    label5.Text = "总帧数【" + bufferCount + "】";
                }
                SendBuffer = new byte[bufferCount * 256];
                ...
                crc = get_crc32(SendBuffer, (uint)SendBuffer.Length);
                br.Close();
                fs.Close();
            }
            catch (Exception ex)
        ...
        private bool CheckInput()
        {
            ...
            if (txt_sb.Text.Trim() == "")
            {
                MessageBox.Show("升级文件 不能为空");
                return false;
            }
            if (!double.TryParse(txt_softVersion.Text, out tempInt))
            {
                MessageBox.Show("升级的软件版本号 不能为空");
                return false;
            }
            if (!double.TryParse(txt_hardVersion.Text, out tempInt))
            {
                MessageBox.Show("需升级的硬件版本号 不能为空");
                return false;
            }
            if (!double.TryParse(txt_maxVersion.Text, out tempInt))
            {
                MessageBox.Show("升级版本上限 不能为空");
                return false;
            }
            if (!double.TryParse(txt_minVersion.Text, out tempInt))
            {
                MessageBox.Show("升级版本下限 不能为空");
                return false;
            }

            if (!double.TryParse(cmb_type.Text.Split('.')[0], out tempInt))
            {
                MessageBox.Show("升级的设备类型 不能为空");
                return false;
            }
            return true;
        }
        ...
        private string GetUpdateState(int updateState)
        {
            string str = "未知";
            switch (updateState)
            {
                case 0:
                    str = "分站未进入升级状态且不能接受升级请求";
                    break;
                case 1:
                    str = "分站未进入升级状态但可以接受升级请求";
                    break;
                case 2:
                    str = "分站已处于升级文件接收阶段";
                    break;
        ...
        private List<int> GetSelectFzh()
        {
            ...
            for (int i = 0; i < dgv.Rows.Count; i++)
            {
                ...
                {
                    items.Add(Convert.ToInt32(dgv.Rows[i].Cells["dz"].Value));
                }
            }
            return items;
        }

        private void UpdateDgvShow(int fzh,string colName,string txt)
        {
            for (int i = 0; i < dgv.Rows.Count; i++)
            {
                if (dgv.Rows[i].Cells["dz"].Value.ToString() == fzh.ToString())
                {
                    dgv.Rows[i].Cells[colName].Value = txt;
                    break;
                }
            }
        }
        ...
        private void btn_cancleUpdate_Click(object sender, EventArgs e)
        {
            Btn_Click(4);
        }
        ...
        private void btn_showRecive_Click(object sender, EventArgs e)
        {
           // Btn_Click(6);
            List<int> fzItems = GetSelectFzh();
            for (int i = 0; i < fzItems.Count; i++)
            {
                UpdateOrderForUserRequest updateOrderForUserRequest = new UpdateOrderForUserRequest();
                updateOrderForUserRequest.fzh = fzItems[i];
                updateOrderForUserRequest.order = 6;
                var result = stationUpdateService.UpdateStationItemForUser(updateOrderForUserRequest);
                UpdateDgvShow(fzItems[i], "jd", result.Message);
            }
        }

        /// <summary>
        /// 1 请求升级，2 下发文件，3 重启升级，4 取消升级，5 版本还原，6 获取版本
        /// </summary>
        /// <param name="order"></param>
        private void Btn_Click(int order)
        {
            if (CheckInput())
            {
                List<int> fzItems = GetSelectFzh();
                for (int i = 0; i < fzItems.Count; i++)
                {
                    UpdateOrderForUserRequest updateOrderForUserRequest = new UpdateOrderForUserRequest();
                    updateOrderForUserRequest.fzh = fzItems[i];
                    updateOrderForUserRequest.order = order;
                    var result = stationUpdateService.UpdateStationItemForUser(updateOrderForUserRequest);
                    UpdateDgvShow(fzItems[i], "jd", result.Message);
                }
            }
        ...
        private void StationUpdate_FormClosed(object sender, FormClosedEventArgs e)
        {
            isRun = false;
        }

        private void cb_refrush_CheckedChanged(object sender, EventArgs e)
        {
            isRun = !isRun;
            if (isRun)
            {
                Thread t = new Thread(RefDgv);
                t.Start();
            }
        }
